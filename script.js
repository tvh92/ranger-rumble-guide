...
function talentDetailMarkup(heroKey,index,mod,requirement){
  ...
  const detailCopy=mod.effect||mod.menu;
  return `<div class="talent-detail-meta">Talent ${index+1} · Unlocks at level ${requirement.level}</div><h4 id="${titleId}">${safe(mod.name)}</h4><div class="talent-detail-icon"><img src="${assetUrl(`talents/${encodeURIComponent(mod.icon)}`)}" alt=""></div><p class="talent-detail-summary">${safe(mod.menu)}</p>${mod.effect?`<div class="talent-effect"><span>Detailed effect</span><p>${safe(detailCopy)}</p></div>`:''}${mod.note?`<p class="talent-note">${safe(mod.note)}</p>`:''}${modTable(mod)}<div class="talent-cost"><span>Core cost</span><strong>${requirement.cores} ${requirement.cores===1?'core':'cores'}</strong></div>`;
}
...
function modsSection(heroKey) {
  ...
  const nodes=mods.map((mod,index)=>{const requirement=talentRequirements[index]||talentRequirements.at(-1);return`<button class="talent-node${index===0?' is-active':''}" type="button" role="tab" aria-controls="talent-detail-${norm(heroKey)}" aria-selected="${index===0?'true':'false'}" data-talent-index="${index}"><span class="talent-unlock"><strong>${requirement.level}</strong><small>${requirement.cores} ${requirement.cores===1?'core':'cores'}</small></span><span class="talent-node-card"><span class="talent-node-icon"><img src="${assetUrl(`talents/${encodeURIComponent(mod.icon)}`)}" alt=""></span><span class="talent-node-copy"><span class="talent-node-type">${talentType(index)}</span><strong>${safe(mod.name)}</strong><span class="talent-node-summary">${safe(mod.menu)}</span></span></span></button>`}).join('');
  return `<section class="mods-section" id="talents"><div class="talent-heading"><h3>Talent progression</h3><p>Select a talent node to inspect</p></div><div class="talent-tree" data-talent-tree data-hero="${safe(heroKey)}" data-active-talent="0" data-pinned-talent="0"><div class="talent-list" role="tablist" aria-label="${safe(displayHeroName(heroKey))} talent progression">${nodes}</div><div class="talent-detail" id="talent-detail-${norm(heroKey)}" role="tabpanel" aria-live="polite" aria-labelledby="talent-title-${norm(heroKey)}-0">${talentDetailMarkup(heroKey,0,mods[0],talentRequirements[0])}</div></div></section>`;
}
...
document.addEventListener('mouseout',event=>{const talent=event.target.closest('.talent-node');if(talent&&!talent.contains(event.relatedTarget)){const tree=talent.closest('[data-talent-tree]');selectTalent(tree,Number(tree.dataset.pinnedTalent));}});
...
document.addEventListener('focusout',event=>{const talent=event.target.closest('.talent-node');if(talent&&!talent.contains(event.relatedTarget)){const tree=talent.closest('[data-talent-tree]');selectTalent(tree,Number(tree.dataset.pinnedTalent));}});
