...
function setSkinPreview(file,name) { const preview=$('#skin-preview-image'); if(!preview)return; const alignment=skinAlignments[file]||{}; preview.classList.toggle('skin-preview-calibrated',/^(Lil'Ann|Sparky|Chip)\//.test(file)); preview.src=skinSource(file); preview.style.setProperty('--skin-scale',alignment.scale||1); preview.style.setProperty('--skin-x',`${alignment.x||0}px`); preview.style.setProperty('--skin-y',`${alignment.y||0}px`); $('#selected-skin-name').textContent=name; }
