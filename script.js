...
function displayStatLabel(column,mode,itemName='') { if(norm(itemName)==='drilldash'){if(column==='Aoe Damage')return'Dash hit damage';if(column==='LastHitAoe Damage')return'Finisher damage';}return column==='Agent Explosion Damage'?'Damage':statLabel(column,mode); }
...
function combinedStatsTable(hero,weapon,gadget,loadout) {
  ...
  const fixedMarkup=fixedGroups.length?`<div class="combined-fixed">${fixedGroups.map(group=>`<dl class="fixed-group"><div class="fixed-group-title">${safe(group.label)}: ${safe(group.displayName)}</div>${group.fixed.map(column=>{const label=displayStatLabel(column,group.mode,group.item?.name);return`<div class="fixed-stat"><dt>${statIcon(label)}<span>${safe(label)}</span></dt><dd>${statValue(column,group.item.levels[0][column])}</dd></div>`}).join('')}</dl>`).join('')}</div>`:'';
