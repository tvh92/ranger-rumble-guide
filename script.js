...
function showDescription(category,name,hero='',full=false) { const panel=$('#profile-description'); if(!panel)return; const label={heroes:'Hero',weapons:'Weapon',gadgets:'Gadget',melee:'Melee',ultimates:'Ultimate'}[category]||category,entry=descriptionEntry(category,name,hero); panel.dataset.category=category;panel.dataset.name=name;panel.dataset.hero=hero; const copy=entry?(full&&entry.full?entry.full:entry.short):'No description is available yet.'; const toggle=entry?.full&&entry.full!==entry.short?`<button class="description-toggle" type="button" data-full="${full?'false':'true'}">${full?'Show short description':'Read full description'}</button>`:''; panel.innerHTML=`<span class="type">${safe(label)}</span><h3>${safe(name)}</h3><p>${safe(copy)}</p>${toggle}`; }
const trim = item => ({...item, levels:item.levels.filter(level => level.Level <= 10)});
const data = {characters:raw.characters.filter(x=>x.name!=='Avatar').map(trim), weapons:[...raw.weapons.map(trim),{name:'Cryoshot',levels:[]},{name:'Toxic Splatter',levels:[]}], gadgets:raw.gadgets.map(trim)};
...
function heroImage(key) { if(key==='Ratchet')return skinSource('_Unassigned/Ratchet_00.png'); const defaultSkin=(window.RANGER_SKINS||[]).find(file=>file===`${key}/${key}.png`); return skinSource(defaultSkin || `${key}.png`); }
...
function card(key, archived=false, type='hero') {
  const l=loadouts[key], title=key,profile=heroProfiles[key];
...
function render() { $('#character-grid').innerHTML=Object.keys(loadouts).map(key=>card(key)).join(''); showArchive('heroes'); }
...
function heroSkins(key) { const def=`${key}/${key}.png`; return (window.RANGER_SKINS||[]).filter(file=>file===def||file.replace(/\.png$/i,'').endsWith(` ${key}`)).sort((a,b)=>a===def?-1:b===def?1:a.localeCompare(b)); }
...
function showHero(key) {
  ...
  $('#dialog-content').innerHTML=`<div class="hero-detail" style="--rarity:${profile.color}"><div class="detail-art"><img id="hero-art" src="${heroRender(key)}" alt="${safe(key)} render"><div class="selected-skin"><h3>Skins</h3><img id="skin-preview-image" src="${heroImage(key)}" alt="${safe(key)} selected skin"><span id="selected-skin-name">Default</span></div></div><div class="detail-body"><button class="profile-title hero-summary" type="button" data-hero="${safe(key)}"><img class="hero-portrait" src="${heroPortrait(key)}" alt=""><div><div class="profile-meta"><span class="rarity-badge">${profile.rarity}</span><span class="type">${safe(heroClasses[key])}</span><span class="speed-badge"><img src="images/UI%20icons/ico_stats_speed.png" alt="">${profile.speed} speed</span></div><h2>${safe(key)}</h2><small>Hero overview</small></div></button><div class="description-panel" id="profile-description"></div><div class="icon-row">${combatIcon('Weapon',l[0],l[1],'weapons',key)}${combatIcon('Gadget',l[2],l[3],'gadgets',key)}${combatIcon('Melee',l[5],l[6],'melee',key)}${combatIcon('Ultimate',l[7],l[8],'ultimates',key,key==='Sparky'?'BigBoomRemote':null)}</div><div class="skins"><h3>Choose skin</h3><div class="skin-row">${skinButtons}</div></div>${combinedStatsTable(hero,weapon,gadget,l)}</div></div>`;
...
render();
let imageTarget;
document.addEventListener('click',event=>{ const cardTarget=event.target.closest('.card'); if(cardTarget&&cardTarget.dataset.type==='hero')showHero(cardTarget.dataset.key); const skin=event.target.closest('.skin'); if(skin){$('#skin-preview-image').src=skinSource(skin.dataset.file);$('#selected-skin-name').textContent=skin.querySelector('span').textContent;} const heroSummary=event.target.closest('.hero-summary'); if(heroSummary)showDescription('heroes',heroSummary.dataset.hero,heroSummary.dataset.hero); const combat=event.target.closest('.combat-icon'); if(combat)showDescription(combat.dataset.descriptionCategory,combat.dataset.descriptionName,combat.dataset.descriptionHero); const toggle=event.target.closest('.description-toggle'); if(toggle){const panel=$('#profile-description');showDescription(panel.dataset.category,panel.dataset.name,panel.dataset.hero,toggle.dataset.full==='true');} const assign=event.target.closest('.assign-image'); if(assign){imageTarget=assign;$('#image-picker').click();} if(event.target.matches('.close'))event.target.closest('dialog').close(); if(event.target.matches('dialog'))event.target.close(); if(event.target.matches('[data-archive]'))showArchive(event.target.dataset.archive); });
