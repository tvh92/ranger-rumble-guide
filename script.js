...
function combinedStatsTable(hero,weapon,gadget,loadout) {
  ...
  return `<section class="combined-stats"><h3>Combat stats</h3>${fixedMarkup}<div class="combined-table"><table class="stats"><caption>Level 1–10 combat statistics for ${safe(hero.name)}</caption><thead><tr><th rowspan="${nested?3:2}" scope="col">Level</th>${headGroups}</tr><tr>${headColumns}</tr>${nested?`<tr>${nestedColumns}</tr>`:''}</thead><tbody>${rows}</tbody></table></div></section>`;
}
...
function modsSection(heroKey) {
  ...
  return `<section class="mods-section"><h3>Mods</h3><div class="mod-grid">${cards}</div></section>`;
}
