...
function previewDescription(category,name,hero){const panel=$('#profile-description');if(!panel)return;const previousHeight=panel.offsetHeight;showDescription(category,name,hero);panel.style.minHeight=`${Math.max(previousHeight,panel.offsetHeight)}px`;}
