...
function displayStatLabel(column,mode) { return column==='Agent Explosion Damage'?'Damage':statLabel(column,mode); }
function statHeader(label,heroName) { return label==='Overhealth'&&heroName==='Lump'?'Over-<br>health':safe(label); }
function statValue(column,value) { const formatted=Number.isInteger(value)?value.toLocaleString():safe(value); return column==='Agent Lifetime'?`${formatted}s`:formatted; }
...
function combinedStatsTable(hero,weapon,gadget,loadout) {
  const groups=[{label:'Hero',displayName:hero.name,item:hero,mode:'hero'},{label:'Weapon',displayName:loadout[0],item:weapon,mode:'normal'},{label:'Gadget',displayName:loadout[2],item:gadget,mode:'normal'},{label:'Ultimate',displayName:loadout[7],item:hero,mode:'ultimate'}].map(group=>{
    const hidden={gloveofdoom:/^(More Agent Drones|Charges)$/i,amoeboidlauncher:/^(AcidZone Damage|AcidZone Duration|Charges)$/i}[norm(group.item?.name)];
  ...
  const fixedMarkup=fixedGroups.length?`<div class="combined-fixed">${fixedGroups.map(group=>`<dl class="fixed-group"><div class="fixed-group-title">${safe(group.label)}: ${safe(group.displayName)}</div>${group.fixed.map(column=>{const label=displayStatLabel(column,group.mode);return`<div class="fixed-stat"><dt>${statIcon(label)}<span>${safe(label)}</span></dt><dd>${statValue(column,group.item.levels[0][column])}</dd></div>`}).join('')}</dl>`).join('')}</div>`:'';
  ...
  const headColumns=visible.map(group=>{if(norm(group.item?.name)==='amoeboidlauncher'){return['Damage','HP'].map((label,index)=>`<th id="${group.id}-${label.toLowerCase()}" class="${index===0?'group-start':''}" scope="colgroup" colspan="3">${label}</th>`).join('')}return group.columns.map((column,index)=>{const label=displayStatLabel(column.key,group.mode).replace(new RegExp(`^${group.label} `,'i'),'').replace(/^Tank /i,'');return`<th id="${column.id}" class="${index===0?'group-start':''}" scope="col"${nested?' rowspan="2"':''}><span class="column-label">${statHeader(label,hero.name)}</span></th>`}).join('')}).join('');
...
function modsSection(heroKey) {
  ...
  const cards=mods.map(mod=>`<article class="mod-card"><header><img src="images/mods/${encodeURIComponent(mod.icon)}?v=${assetVersion}" alt=""><div><span>Hero mod</span><h4>${safe(mod.name)}</h4></div></header><p class="mod-menu">${safe(mod.menu)}</p><p class="mod-effect">${safe(mod.effect)}</p>${mod.note?`<p class="mod-note">${safe(mod.note)}</p>`:''}${modTable(mod)}</article>`).join('');
  return `<section class="mods-section" id="mods"><h3>Mods</h3><div class="mod-grid">${cards}</div></section>`;
}
