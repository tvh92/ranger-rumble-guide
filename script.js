...
function combinedStatsTable(hero,weapon,gadget,loadout) {
  ...
  const fixedMarkup=fixedGroups.length?`<div class="combined-fixed${stackedFixed?' fixed-stacked':''}">${fixedGroups.map(group=>{const amoeboid=norm(group.item?.name)==='amoeboidlauncher',lifetimeColumns=['Small Lifetime','Medium Lifetime','Large Lifetime'],columns=amoeboid?group.fixed.filter(column=>!lifetimeColumns.includes(column)):group.fixed,lifetime=amoeboid?`<div class="fixed-stat lifetime-stat"><dt>${statIcon('Lifetime')}<span>Lifetime</span></dt><dd>${lifetimeColumns.map((column,index)=>`${group.item.levels[0][column]}s ${'SML'[index]}`).join(' | ')}</dd></div>`:'',stagedFireRate=group.label==='Weapon'&&norm(group.item?.name)==='blackholestorm'?`<div class="fixed-stat staged-fire-rate"><dt>${statIcon('Fire rate')}<span>Fire rate</span></dt><dd><span>1×</span><i>→</i><span>3× <small>at 1.3s</small></span><i>→</i><span>5× <small>at 3s</small></span><i>→</i><span>8× <small>at 5s (talent)</small></span></dd></div>`:'';return`<dl class="fixed-group"><div class="fixed-group-title">${safe(group.label)}: ${safe(group.displayName)}</div>${columns.map(column=>{const label=displayStatLabel(column,group.mode,group.item?.name,group.displayName);return`<div class="fixed-stat"><dt>${statIcon(label)}<span>${safe(label)}</span></dt><dd>${statValue(column,group.item.levels[0][column],group.item?.name)}</dd></div>`}).join('')}${stagedFireRate}${lifetime}</dl>`}).join('')}</div>`:'';
